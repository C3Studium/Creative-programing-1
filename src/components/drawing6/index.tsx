...
export default function Drawing6() {
    ...
    const sketch = () => {
        ...
        const numCircles = Array.from({ length: circles }, () => ({ slices: random.rangeFloor(5, 20) }));
        const radius = 150;

        let lineWidths = [] as number[];
        let bins = [] as number[];

        let bin, mapped, lineWidth;
        ...
        for (let i = 0; i < numCircles.length; i++) {
            const t = i / (numCircles.length - 1)
            lineWidth = eases.sineInOut(t) * 200 + 20;
            lineWidths.push(lineWidth)

            // Generate bins for this specific circle's slices
            for (let j = 0; j < numCircles[i].slices; j++) {
                bin = random.rangeFloor(1, 1024);
                if (random.value() > 0.75) bin = 0;
                bins.push(bin);
            }
        }
        ...
        return ({ context, width, height }: { context: CanvasRenderingContext2D, width: number, height: number, frame: number, stroke: string }) => {
            ...
            for (let i = 0; i < numCircles.length; i++) {
                ...
                const slices = numCircles[i].slices;
                const sliceAngle = Math.PI * 2 / slices;

                for (let j = 0; j < slices; j++) {
                    context.rotate(sliceAngle);

                    // Grab the next bin in the flat array
                    bin = bins[binIndex];
                    binIndex++;

                    if (!bin) continue;

                    mapped = math.mapRange(audioDataRef.current[bin], minDB.current, maxDB.current, 0, 1.25, true);
                    ...
                    context.beginPath();
                    context.arc(0, 0, cRadius + context.lineWidth * 0.5, 0, sliceAngle);
                    context.stroke();
                }
